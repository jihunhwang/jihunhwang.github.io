...
function updateWidgetSrc() {
  const widget = document.getElementById("ctf-stats-widget");
  if (!widget) return;

  const rawTheme = document.documentElement.getAttribute("data-theme");
  const isDark = (rawTheme === "dark");

  const baseUrl = "https://ctf.neilhommes.xyz/?teamid=11464";
  const finalUrl = isDark ? `${baseUrl}&theme=dark` : baseUrl;

  if (widget.src !== finalUrl) {
    widget.src = finalUrl;
    // console.log("Widget src updated due to theme change:", finalUrl);
  }
}

// Run once on page load
window.onload = updateWidgetSrc;
