---
title: "b01lers Bootcamp CTF 2023 Writeup"
date: 2023-10-25
description: b01lers Bootcamp CTF 2023 Writeup
tags:
  - CTF
toc: true
mathjax: yes
...
<p><a href="https://b01lers.com/">b01lers</a> hosts a series of CTF bootcamp workshops that serves an introductory crash course on CTF topics (web, rev, crypto, and pwn) and basic techniques for new members every fall semester. <a href="https://bootcamp.b01lers.com/">Bootcamp CTF</a> is then held afterwards, as some sort of a 'final exam' of the course.</p>

<p><a href="https://bootcamp.b01lers.com/">Bootcamp CTF</a> was the first CTF competition that I have ever been to (in 2022, with enigcryptist, markroxor, and Abe), and I have been an active (try-my-best-to-be) member of <a href="https://b01lers.com/">b01lers</a> ever since then. This CTF hence means a lot to me. I also personally think the quality of problems in <a href="https://bootcamp.b01lers.com/">b01lers Bootcamp CTF</a> surpasses any other beginner-friendly CTFs, and they represent a good range of various topics and difficulties.</p>
...
$$
\texttt{tag}' := (\texttt{id} + N)^d = \texttt{id}^d + d \cdot \texttt{id}^{d-1} N + \cdots + N^d = \texttt{id}^d \; \text{ mod } N
$$
Hence, \( (\texttt{id} + N, \texttt{tag}') \) is a valid forgery: \( \texttt{id} \neq \texttt{id} + N \) (note that <code>tag.py</code> does not take \( \text{mod } N\), it just cheks whether they are exactly the same or not) yet  \( \texttt{tag} = \texttt{tag}' \). We do not need to compute \( \texttt{tag}' \) (in fact, we cannot since we do not have \( d \) which is the private key) ourselves, we just need to compute and feed in \( \texttt{id} + N \) to the console. 
</p>
...
<center>
<img src="/image/btcp_2023/cbc_crypt.jpg">
</center>

<font size="4">
<p>Let our plaintext \( m = 00...00 \) be 16 \(00\)'s. It will then padded as 16 \(00\)'s and 16 \(16\)'s (\( 10\)'s, in hex) \( m_{\text{pad}} = 00...00 \| 10...10\). Each block should have length 16 since it is AES-CBC. The block of \( m\) would be \( m_1 = 00...00\) and the second block would be \( m_2 = 10...10\). Then the corresponding blocks of ciphertexts \( c_1\) and \( c_2\) would be:
$$
\begin{align*}
...
<p>Hence, we should figure out a way to 'hijack' the <code>user_id</code> of admin. This can be done easily with webhook (for 'eavesdropping' HTML requests) and a JS script that grabs the <code>document.cookie</code> of the visitor. Then, once we give its post number to the admin, then upon their visit, their cookies will be transmitted to our webhook page. </p>
</font>
...
<font size="4">
<p>There it is! We get the new <code>user_id</code> value that definitely do not belong to us. Changing our <code>user_id</code> to that on the developer tools gives us access to the someone else's account, and that turns out to be admin!</p>
...
<center>
<img src="/image/btcp_2023/sus_6.png">
</center>
<p></p>
<center>
<img src="/image/btcp_2023/sus_7.png">
</center>
