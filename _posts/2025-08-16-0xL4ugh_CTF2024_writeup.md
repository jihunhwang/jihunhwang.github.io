...
There is also <a href="https://github.com/pycountry/pycountry">pycountry</a> package that contains the name of all countries and their details. 
The only thing to be careful in that package is that some countries are names slightly differently; for example, Russia as Russian Federation and Turkey as Turkiye. 
I was told that Turkiye is actually the correct spelling, but apparently the chal authors did not use that spelling.
I realized that after failing even after trying <code>pycountry.historic_countries</code> that returns the name of countries that does not exist (e.g., USSR).  
</p>
</font>
