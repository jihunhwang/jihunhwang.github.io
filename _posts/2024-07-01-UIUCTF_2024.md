---
title: "UIUCTF 2024 Groups Writeup"
date: 2024-07-01
...
<font size="4">
<p> 
This weekend, I played UIUCTF with b01lers. I was not planning on playing it initially because I had to finish up some research work and internship stuffs, but someone pinged me in the b01lers Discord server asking if I have any thoughts on this particular challenge.</p>

<p>As a member of b01lers, I am meant to have some geopolitical rivalries (? lol) with <a href="https://sigpwny.com/">SIGPwny</a> (the team that hosts UIUCTF).<sup><a href="#fn1" id="ref1">1</a></sup> But near the end, I wished I could spend more time on this competition because I was able to tell just by reading the challenges that they are written very well and with care. So, thank you, SIGPwny, for the great opportunity, and for giving me a productive yet reinvigorating break from my grad school and intern work.</p>

<p>As of right now, the <a href="https://2024.uiuc.tf/">CTF website</a> is still alive. <a href="https://ctftime.org/event/2275">Click this</a> for the CTFTime page. </p>
...
<p>Once we pass this initial screening, we are asked to solve a discrete log problem. The server chooses a random $a \in \mathbb{Z}_n^\times$ and $2 \leq k \leq n-1$, returns $a$ and $a^k$, and asks us to figure out $k$.</p>
...
<p>By Fermat's little theorem, if $n$ is prime then $a^{n-1} \equiv 1 \; (\text{mod } n)$ holds for all $a \in \mathbb{Z}_n^\times$, but unfortunately we are forced to pick a composite number for $n$. The program tests whether $a \in \mathbb{Z}_n^\times$ or not 50 times only, but since $a$ is picked randomly, it is likely very difficult to create a custom $n$ that passes the test exactly 50 times or higher; that is, giving an $n$ that actually satisfies the equation $a^{n-1} \equiv 1 \; (\text{mod } n)$ for all $a$, albeit not prime, is desired. </p>
</font>
...
###### Attempt 1
<font size="4">
<p>The converse of Fermat's little theorem is not true in general. There are composite numbers where Fermat's little theorem still holds and they are called, as the challenge description spoiled, <a href="https://en.wikipedia.org/wiki/Carmichael_number">Carmichael numbers</a>.</p>
...
\end{align*}  
\]
<p>which gives some very large number $n = 2887\dots5867$. Giving this number as input to the server works, but the problem is, we have to solve a discret log problem:</p>
{% highlight bash %}
...
<p>I could not find any list of large Carmichael numbers (512-bit or larger) anywhere online including OEIS and research papers. Then, I found <a href="https://math.stackexchange.com/questions/3029794/large-carmichael-number">this StackExchange post</a> that gives the following Carmichael number:</p>
\[
\begin{align*}
...
<p>The correctness of this algorithm comes directly from the fact that $n$ is a Carmichael number if $\lambda(n) \mid n-1$ where $\lambda(n)$ is the <a href="https://en.wikipedia.org/wiki/Carmichael_function">Carmichael function</a> defined to be the LCM of the prime factor minus one's of $n$.</p>

<p>Now the question is: What $\Lambda$'s should we choose? It should be a value of $\lambda(n)$ for some $n$, but all the $\Lambda$'s in that paper by Guillaume and Morain seem too large.</p>
...
<p>But note that we might not need a large $\Lambda$. We want $n = \prod_{p \in S} p$ to be large enough, and that might be achievable even with a not-very-large $\Lambda$. Then I came across <a href="https://www.ams.org/journals/mcom/1996-65-214/S0025-5718-96-00692-8/S0025-5718-96-00692-8.pdf">this paper</a> by Loh and Neibuhr that has lists of small $\Lambda$'s. I just picked one that looks still large enough and ran my code again, then voila: </p>
...
<p>As intended/hoped, generating this output takes only a few seconds. Now we just need to get it all together, and we are done!</p>
